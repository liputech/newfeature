...
if ( !class_exists( 'NewfeatureTheme' ) ) {

	class NewfeatureTheme {
		...
		public function redux_init() {
			...
			add_filter( 'redux/newfeature/aURL_filter', '__return_empty_string' ); // Remove Redux Ads
			...
			add_action( 'redux/options/newfeature/saved', array( $this, 'flush_redux_saved' ), 10, 2 );
			add_action( 'redux/options/newfeature/section/reset', array( $this, 'flush_redux_reset' ) );
			add_action( 'redux/options/newfeature/reset', array( $this, 'flush_redux_reset' ) );
		}
		...
		public function flush_redux_saved( $saved_options, $changed_options ){
			...
			if ( $flush ) {
				update_option( 'newfeature_rewrite_flash', true );
			}
		}
		...
		public function rewrite_flush_check() {
			if ( get_option('newfeature_rewrite_flash') == true ) {
				flush_rewrite_rules();
				update_option( 'newfeature_rewrite_flash', false );
			}
		}
	}
}

NewfeatureTheme::instance();
