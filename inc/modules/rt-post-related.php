...
if( ! function_exists( 'newfeature_related_post' )){
	
	function newfeature_related_post(){
		$thumb_size = 'newfeature-size7';
		$post_id = get_the_id();	
		$number_of_avail_post = '';
		$current_post = array( $post_id );	
		$title_length = NewfeatureTheme::$options['show_related_post_title_limit'] ? NewfeatureTheme::$options['show_related_post_title_limit'] : '';
		$related_post_number = NewfeatureTheme::$options['show_related_post_number'];
		...
		<div class="owl-wrap rt-related-post related post <?php echo esc_attr( $wrapper_class );?>">
			<div class="title-section">
				<h3 class="related-title"><?php esc_html_e ( 'Related Post', 'newfeature' ); ?></h3>
				<?php if ( $count_post > 3 ){ ?>
									...
									<?php the_post_thumbnail( $thumb_size, ['class' => 'img-responsive'] ); ?>
										...
										if ( NewfeatureTheme::$options['display_no_preview_image'] == '1' ) {
											...
											elseif ( empty( NewfeatureTheme::$options['no_preview_image']['id'] ) ) {
												$thumbnail = '<img class="wp-post-image" src="'.NEWFEATURE_IMG_URL.'noimage_420X435.jpg" alt="'. the_title_attribute( array( 'echo'=> false ) ) .'">';
											}
