...
if( ! function_exists( 'newfeature_breadcrumbs' )){

	function newfeature_breadcrumbs(){
			
		if( NewfeatureTheme::$options['breadcrumb_active'] == 0 ) {			
			return;
		}
		...
		$home_text  = esc_html__( 'Home', 'newfeature' );
		...
		if ( ! is_home() && ! is_front_page() || is_paged() ){
			...
			elseif ( is_singular() ){			
				...
				if ( get_post_type() == 'post' ){
					...
					$category = newfeature_get_primary_category_id();
					$category = get_category( $category );
